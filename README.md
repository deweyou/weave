...
支持标题、加粗、斜体、删除线、链接、行内/围栏代码、列表、引用和勾选标记。列表为文本标记，支持无序列表和引用续行，没有嵌套结构或可点击任务；表格和图片不在本期范围。排版会替换 Markdown 源文，也会重置所选范围既有样式，不提供无损 Markdown 往返。
旧纯文本 JSON 可直接读取，首次编辑保存增加富文本字段。格式使用 SwiftUI 属性编码，保留原正文投影用于搜索。

## 边界

目前支持基础富文本，尚无删除、CloudKit 同步、待办或画板。保存采用简单 JSON 全量写入，适合本期小规模文本验证；大规模数据与同步接入前需要重新验证存储性能和迁移策略。
