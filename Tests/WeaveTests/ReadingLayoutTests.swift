...
struct ReadingLayoutTests {
    ...
    @Test @MainActor func quoteBarDoesNotEnterFollowingEmptyBodyCaretRow() throws {
        ...
        let quoteStyle = try #require(storage.attribute(.paragraphStyle, at: quoteRange.location, effectiveRange: nil) as? NSParagraphStyle)
    ...
    @Test @MainActor func emptyQuoteHasAVisibleBarWithoutPlaceholderText() {
        ...
        #expect(bar.width == 2)
        #expect(abs(bar.height - (NativeTextAttributes.stableLineHeight(for: font)
    ...
    @Test @MainActor func emptyQuoteBarCanUseAnEmptyParagraphInsideANonemptyDocument() {
        let storage = NSTextStorage(attributedString: NativeTextAttributes.native(
        ...
        let font = storage.attribute(.font, at: insertion, effectiveRange: nil) as? NSFont
        ...
        #expect(abs(bar.minY - (fragment.minY
            - font.pointSize * (DocumentTypography.quoteBarVerticalOvershootRatio + DocumentTypography.quoteBarOpticalRiseRatio))) < 0.01)
    }

    @Test @MainActor func leadingInlineCodeReservesItsScaledLeftMargin() throws {
        for prefix in ["", "# "] {
            let storage = NSTextStorage(attributedString: NativeTextAttributes.native(MarkdownFormatting.render(prefix + "`value` after"), context: EnvironmentValues().fontResolutionContext))
            let layout = CodeLayoutManager()
            let container = NSTextContainer(size: CGSize(width: 500, height: 10_000))
            storage.addLayoutManager(layout)
            layout.addTextContainer(container)
    ...
    @Test @MainActor func inlineCodeScalesWithEveryHeadingAndSurvivesRoundTrip() throws {
        ...
        for level in 0...6 {
            ...
            let expectedSize = DocumentTypography.bodySize * (DocumentTypography.heading(for: role)?.ratio ?? 1) * DocumentTypography.inlineCodeScale
    ...
    @Test @MainActor func inlineCodeColorFollowsAppearanceWithoutReopeningDocument() throws {
        ...
        light.performAsCurrentDrawingAppearance {
            native = NativeTextAttributes.native(MarkdownFormatting.render("`let value = 1`"), context: EnvironmentValues().fontResolutionContext)
        }
    ...
    @Test @MainActor func inlineBackgroundUsesCodeMetricsInsideTallText() throws {
        let native = NativeTextAttributes.native(MarkdownFormatting.render("# Heading `let counthg = 42`"), context: EnvironmentValues().fontResolutionContext)
        let storage = NSTextStorage(attributedString: native)
        let layout = CodeLayoutManager()
        let container = NSTextContainer(size: CGSize(width: 500, height: 10_000))
        storage.addLayoutManager(layout)
    ...
    @Test @MainActor func inlineCodeSingleSpacesDoNotDoubleBoundaryGaps() {
        ...
        for source in ["\u{524D}`x`\u{540E}", "\u{524D} `x` \u{540E}"] {
            ...
            layout.addTextContainer(container)
            layout.ensureLayout(for: container)
            positions.append(["x", "\u{540E}"].map { word in
    ...
    @Test @MainActor func disconnectedCodeSurfacesDoNotOverlap() throws {
        ...
        storage.enumerateAttribute(.weaveCodeStyle, in: NSRange(location: 0, length: storage.length)) { value, range, _ in
            ...
            surfaces.append(layout.codeBackgroundRect(forGlyphRange: layout.glyphRange(forCharacterRange: range, actualCharacterRange: nil), in: container))
        }
    ...
    @Test @MainActor func codeSurfaceDoesNotResizeWhenTypingAttributesChange() throws {
        ...
        for index in [0, storage.length, 3, storage.length] {
            ...
            for attributes in [storage.attributes(at: 0, effectiveRange: nil), NativeTextAttributes.native(AttributedString("Body"), context: context).attributes(at: 0, effectiveRange: nil)] {
    ...
    @Test @MainActor func nativeSelectionExcludesSpacingAndCodeHeader() throws {
        for source in ["Body text\n\nNext", "\u{6B63}\u{6587} text\n\nNext", "## Heading\n\nNext", "```swift\nlet value = 1\nlet next = 2\n```", "Before\n\n\nNext", "Emoji \u{1F600}\n\nNext"] {
            let storage = NSTextStorage(attributedString: NativeTextAttributes.native(MarkdownFormatting.render(source), context: EnvironmentValues().fontResolutionContext))
            let layout = CodeLayoutManager()
            ...
            window.contentView = view
            layout.ensureLayout(for: container)
            layout.enumerateLineFragments(forGlyphRange: NSRange(location: 0, length: layout.numberOfGlyphs)) { fragment, used, _, glyphs, _ in
                ...
                layout.enumerateEnclosingRects(forGlyphRange: NSRange(location: glyphs.location, length: 1), withinSelectedGlyphRange: glyphs, in: container) { rect, _ in
    ...
    @Test @MainActor func nativeSelectionKeepsAThirdOfLineSpacingAsBreathingRoom() {
        let storage = NSTextStorage(attributedString: NativeTextAttributes.native(AttributedString("First\u{2028}Second"), context: EnvironmentValues().fontResolutionContext))
        let layout = CodeLayoutManager()
        let container = NSTextContainer(size: CGSize(width: 500, height: 10_000))
        storage.addLayoutManager(layout)
        layout.addTextContainer(container)
    ...
    @Test @MainActor func nativeEmptyParagraphGeometryMatchesTypedText() throws {
        for (prefix, suffix) in [("", "\nNext"), ("Before\n", "\nNext"), ("Before\u{2028}", "\nNext"), ("", ""), ("Before\n", ""), ("Before\u{2028}", "")] {
            ...
            for content in ["", "x", "\u{4E2D}", ""] {
                let context = EnvironmentValues().fontResolutionContext
                let storage = NSTextStorage(attributedString: NativeTextAttributes.native(AttributedString(prefix + content + suffix), context: context))
                let layout = CodeLayoutManager()
                let container = NSTextContainer(size: CGSize(width: 500, height: 10_000))
                storage.addLayoutManager(layout)
                layout.addTextContainer(container)
                ...
                let window = NSWindow(contentRect: view.frame, styleMask: .borderless, backing: .buffered, defer: false)
                window.contentView = view
                view.typingAttributes = NativeTextAttributes.native(AttributedString("x"), context: context).attributes(at: 0, effectiveRange: nil)
    ...
    @Test @MainActor func markedCJKTextKeepsBodyViewportAndLineGeometryStable() throws {
        ...
        let committedBaseline = layout.lineFragmentRect(forGlyphAt: committedGlyph, effectiveRange: nil).minY
    ...
    @Test @MainActor func paragraphBreakAddsEightPointsComparedWithLineBreak() {
        func baselineDistance(_ separator: String) -> CGFloat {
            let text = NativeTextAttributes.native(AttributedString("First" + separator + "Second"), context: EnvironmentValues().fontResolutionContext)
    ...
    @Test @MainActor func headingHierarchyStaysReadableAndUsesConsistentSpacing() throws {
        ...
        for level in 1...6 {
            ...
            previous = size
            let native = NativeTextAttributes.native(MarkdownFormatting.render("Body\n" + String(repeating: "#", count: level) + " Heading"), context: context)
    ...
    @Test(arguments: ["x", "\u{6B63}\u{6587}"]) @MainActor func typingAndDeletingInBlankLineKeepsFollowingContentPosition(typed: String) throws {
        for following in ["Next", "## Next"] {
            let initial = NativeTextAttributes.native(MarkdownFormatting.render("Before\n\n\n" + following), context: EnvironmentValues().fontResolutionContext)
    ...
    @Test(arguments: ["Body", "\u{6B63}\u{6587}"]) @MainActor func blankAndBodyUseTheSameNativeLineHeight(bodyText: String) throws {
        let text = NativeTextAttributes.native(MarkdownFormatting.render(bodyText + "\n\n\nNext"), context: EnvironmentValues().fontResolutionContext)
        let storage = NSTextStorage(attributedString: text)
        let layout = CodeLayoutManager()
        let container = NSTextContainer(size: CGSize(width: 560, height: 10_000))
        storage.addLayoutManager(layout)
    ...
    @Test @MainActor func blankParagraphsDoNotStackMarginsOrLoseTheirNaturalHeight() throws {
        ...
        let heading = try #require(native.attribute(.paragraphStyle, at: string.range(of: "Heading").location, effectiveRange: nil) as? NSParagraphStyle)
        ...
        #expect(MarkdownFormatting.serialize(NativeTextAttributes.rich(native), context: EnvironmentValues().fontResolutionContext) == source)
    }

    @Test @MainActor func explicitBodyRoleDoesNotAcquireHeadingSpacingAtLargeFontSizes() throws {
        let text = NSMutableAttributedString(string: "Large body", attributes: [.font: NSFont.systemFont(ofSize: 32), .weaveParagraphStyle: "body"])
